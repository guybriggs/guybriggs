...
export class WorkAttractionSystem {
  update(world, deltaTime) {
    ...
    const candidateAgents = world.getEntitiesByComponents(['Position', 'Velocity', 'Inventory']);
    ...
    rods.forEach(rodId => {
      ...
      if (nearestAgent) {
        ...
        if (distance < 5) {
          ...
          // Optionally remove randomly move
          if (world.hasComponent(id, 'RandomlyMove')) {
            world.removeComponent(id, 'RandomlyMove');
          }
        } else {
          const speed = 1 / 10;
