...
export class WindArrowSystem {
  ...
  isFullyBuiltWall(tile) {
    // e.g., not transparent & type in [white_bricks, dark_floor, door_tile]
  ...
  spawnRepairWorker(world, row, col, buildingType) {
    ...
    const wkr = createAgent(world, { behavior: BehaviorTypes.WANDER, emotion: 'NEUTRAL' });
    const pos = world.getComponent(wkr, 'Position');
    if (pos){ pos.x=sx; pos.y=sy; }
    const rend = world.getComponent(wkr, 'Renderable');
    if (rend){ rend.helmet=true; }

    // optional: add a "RepairTask" if you want a separate logic
    world.addComponent(wkr, 'ConstructionTask', {
      isGrid:true, row, col,
      startTime:millis(),
      constructing:false, returning:false,
