...
function computeRodPrice(nthRod) {
  // Start from 8, then for i=1..nthRod, add getNextPriceIncrement(i)
  let price = 8;
...
function computeLockerPrice(nthRod) {
  // Start from 8, then for i=1..nthRod, add getNextPriceIncrement(i)
  let price = 8;
  for (let i = 1; i <= nthRod; i++) {
    price += getNextPriceIncrement(i);
...
function computeIceboxPrice(nthRod) {
  // Start from 8, then for i=1..nthRod, add getNextPriceIncrement(i)
  let price = 8;
  for (let i = 1; i <= nthRod; i++) {
    price += getNextPriceIncrement(i);
    ...
    if (demand.lockedRegister) {
      ...
      if (this.moveTowards(world, deltaTime, entity, row, col, 1)) {
        ...
        if (freshInRegister > 0) {
          ...
          // NEW: UTILITY +10
          const consumerUtility = world.getComponent(entity, 'Utility');
          if (consumerUtility) {
            consumerUtility.value += 10;
          }

          // If consumer is also a follower => also add 10 to the player
          if (world.hasComponent(entity, 'Follower')) {
            const playerEnts = world.getEntitiesByComponent('Player');
            if (playerEnts.length > 0) {
              const playerId = playerEnts[0];
              const playerUtility = world.getComponent(playerId, 'Utility');
              if (playerUtility) {
                playerUtility.value += 10;
              }
            }
          }
        }
        // --- (B) Rotten fish => -10 rep but +5 utility
          ...
          const consumerUtility = world.getComponent(entity, 'Utility');
          if (consumerUtility) {
            consumerUtility.value += 5;
          }

          // If consumer is a follower => also +5 to player's utility
          if (world.hasComponent(entity, 'Follower')) {
            const playerEnts = world.getEntitiesByComponent('Player');
            if (playerEnts.length > 0) {
              const pId = playerEnts[0];
              const pUtil = world.getComponent(pId, 'Utility');
              if (pUtil) {
                pUtil.value += 5;
              }
            }
    ...
    switch (tileType) {
      case 'fishingrod': {
        if (this.moveTowards(world, deltaTime, entity, tile.row, tile.col, 1)) {
          ...
          // --- NEW: Increment the current # of rods
      ...
      case 'icebox': {
        if (this.moveTowards(world, deltaTime, entity, tile.row, tile.col, 1)) {
          ...
          // --- NEW: increment the # of iceboxes
          world.numIceboxesCurrent = (world.numIceboxesCurrent || 0) + 1;
          const nthIcebox = world.numIceboxesCurrent;
          const price = computeIceboxPrice(nthIcebox);
      ...
      case 'locker': {
        ...
        if (this.moveTowards(world, deltaTime, entity, tile.row, tile.col, 1)) {
          ...
          // --- NEW: increment the # of lockers
          world.numLockersCurrent = (world.numLockersCurrent || 0) + 1;
          const nthLocker = world.numLockersCurrent;
          const price = computeLockerPrice(nthLocker);
