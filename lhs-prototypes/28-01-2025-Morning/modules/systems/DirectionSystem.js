...
// --------------------------------------------------------------------
  ...
  handleChef(world, entity, dt) {
    ...
    // Count how many fish/potato we have (fresh + wasted)
    const freshFish      = inv.items.fish || 0;
    const wastedFish     = inv.items.wasted_fish || 0;
    const freshPotato    = inv.items.potato || 0;
    const wastedPotato   = inv.items.wasted_potato || 0;
    const totalFish      = freshFish + wastedFish;
    const totalPotato    = freshPotato + wastedPotato;
  
    // Grab references to fryer + fishchips_cash
    const fryer         = this.findAnyTileOfType('fryer');
    const fishchipsCash = this.findAnyTileOfType('fishchips_cash');
  
    // Also look for fish_table + potato_table if we still need to pick up
    let fishTable = null, potatoTable = null;
    if (totalFish === 0 || totalPotato === 0) {
      ...
      // If any key tiles are missing => do nothing
      if (!potatoTable || !fishTable || !fryer || !fishchipsCash) {
        supply.chefState = 'IDLE';
        return;
      }
    } else {
      // We already have everything we need => just confirm fryer + register exist
    ...
    switch (supply.chefState) {
      ...
      // -------------------------------------------------------
      // IDLE => if we already have fish+potato, go straight to fryer
      // else go to potato table first
      // -------------------------------------------------------
      case 'IDLE': {
        if (totalFish > 0 && totalPotato > 0) {
      ...
      // -------------------------------------------------------
      // GO_POTATO_TABLE => pick up fresh if possible, else wasted
      // only if we have 0 totalPotato
      // -------------------------------------------------------
      case 'GO_POTATO_TABLE': {
        ...
        if (arrived) {
          if (totalPotato < 1) {
            ...
            // If no fresh, check wasted
            else if ((tileInv.items.wasted_potato || 0) > 0) {
              tileInv.items.wasted_potato--;
      ...
      // -------------------------------------------------------
      // GO_FISH_TABLE => pick up fresh if possible, else wasted
      // only if we have 0 totalFish
      // -------------------------------------------------------
      case 'GO_FISH_TABLE': {
      ...
      // -------------------------------------------------------
      // GO_FRYER => if we have at least 1 fish (fresh/wasted)
      // and 1 potato (fresh/wasted), we FRY
      // -------------------------------------------------------
      case 'GO_FRYER': {
        const arrived = this.moveTowards(world, dt, entity, fryer.row, fryer.col, 10);
        if (arrived) {
          const updatedFish    = (inv.items.fish || 0) + (inv.items.wasted_fish || 0);
          const updatedPotato  = (inv.items.potato || 0) + (inv.items.wasted_potato || 0);
  
          if (updatedFish > 0 && updatedPotato > 0) {
            // We'll do a 5-second wait => produce fishchips
            supply.chefState = 'FRYING';
            if (waiting) waiting.until = 300; // 5 seconds
          } else {
            // if we somehow lost ingredients => go idle
            supply.chefState = 'IDLE';
          }
        }
        break;
      ...
      // -------------------------------------------------------
      // FRYING => remove 1 from whichever fish/potato we have
      // then produce fishchips => GO_FISHCHIPS_CASH
      // -------------------------------------------------------
      case 'FRYING': {
        if (waiting && waiting.until <= 0) {
          // Remove 1 fish item
          if (inv.items.fish > 0) {
            inv.items.fish--;
          } else if (inv.items.wasted_fish > 0) {
            inv.items.wasted_fish--;
          }
          // Remove 1 potato item
          if (inv.items.potato > 0) {
            inv.items.potato--;
          } else if (inv.items.wasted_potato > 0) {
      ...
      // -------------------------------------------------------
      // GO_FISHCHIPS_CASH => deposit final fishchips => IDLE
      // -------------------------------------------------------
