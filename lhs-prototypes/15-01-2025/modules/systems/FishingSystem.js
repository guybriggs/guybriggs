...
export class FishingSystem {
  ...
  update(deltaTime) {
    ...
    const workers = this.world.getEntitiesByComponents(['Position', 'Velocity', 'Inventory', 'Job']);
    ...
    workers.forEach(workerId => {
      ...
      // Phase 2: Worker carrying fish; go deposit it
        ...
        let nearestRegister = null;
        let minDist = Infinity;
        for (let b of this.buildingSystem.nonGridBuildings) {
          if (b.type === 'cash_register') {  // Look for cash registers instead of crates
        ...
        if (nearestRegister) {
          const dx = (nearestRegister.x + nearestRegister.w / 2) - pos.x;
          const dy = (nearestRegister.y + nearestRegister.h / 2) - pos.y;
          const d = Math.hypot(dx, dy);
          if (d > 1) {
            const speed = 1/10;
            ...
            vel.vx = 0;
            vel.vy = 0;
            //console.log(`Worker ${workerId} is waiting at the cash register.`);
          }
        } else {
          //console.log(`Worker ${workerId} couldn't find a cash register.`);
        }
      }
    });
  }
