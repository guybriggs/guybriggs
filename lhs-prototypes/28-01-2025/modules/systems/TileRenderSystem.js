  ...
  update(world, p5, playerEntity, scaleFactor) {
    ...
    for (let row = startRow; row < endRow; row++) {
      for (let col = startCol; col < endCol; col++) {
        ...
        // 2) Draw item stacks depending on tile type:

        // (A) If tile is fish-related => fish or wasted fish
        if (
          tileType === 'fishingrod' ||
          tileType === 'icebox'     ||
          ...
          if (tile.inventory && tile.inventory.items) {
            ...
            // If fishCount is negative => treat that as negative fish
            if (fishCount < 0) {
        ...
        // (B) If tile is potato-related => potato or wasted potato
        else if (
          tileType === 'potato_cashregister' ||
          tileType === 'potato_table'
        ) {
          if (tile.inventory && tile.inventory.items) {
            ...
            // If potatoCount is negative => treat that as negative
            if (potatoCount < 0) {
        ...
        // (D) Otherwise => optional debug-draw
        else {
          if (tile.inventory) {
            ...
            if (items.length > 0) {
              for (let i = 0; i < items.length; i++) {
                ...
                p5.fill(0);
                p5.text(`${count} ${item}`, col * tileSize, row * tileSize + i * 25);
              }
            }
          }
        }

        // 4) Show supply price if tile is fishingrod / icebox / cashregister / potato_seed / potato_cashregister
        if (
          tileType === 'fishingrod' ||
          tileType === 'icebox'     ||
          tileType === 'cashregister' ||
