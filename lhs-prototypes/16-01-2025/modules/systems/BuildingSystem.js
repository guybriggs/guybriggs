...
export class BuildingSystem {
  ...
  placeGridBuilding(r, c, t) {
    // Only place on grassland, dark_floor, or forest
    if (
      tileMap[r][c].type !== 'grassland' &&
      tileMap[r][c].type !== 'dark_floor' &&
    ...
    // 'key' checks for dark_floor, enclosed BFS, etc.
    if (t === 'key') {
      const tileHere = tileMap[r][c];
      if (tileHere.type !== 'dark_floor') {
        console.log("Can't place a key unless on dark_floor");
        return;
      }
    ...
    let tileType;
    switch (t) {
      case 'floor': tileType = 'dark_floor'; break;
      case 'door':  tileType = 'door_tile';  break;
      case 'bed':   tileType = 'bed_tile';   break;
      case 'table': tileType = 'table_tile'; break;
      case 'key':   tileType = 'key_tile';   break;
      default:      tileType = 'white_bricks';
    }
  ...
  placeFridge(x, y) {
    // 1) Create a new ECS entity for the fridge
    const entityId = this.world.createEntity();
    const width = 40;
    const height = 40;
    
    // 2) Add Position/Renderable/Name (or any components you want)
    this.world.addComponent(entityId, 'Position', { x, y });
    this.world.addComponent(entityId, 'Renderable', {
      width,
      height,
      color: '#A6E7FF'  // some "cool" color for a fridge
    });
    this.world.addComponent(entityId, 'Name', { name: 'Fridge' });

    // 3) Store in nonGridBuildings so you can see it in "draw"
    this.nonGridBuildings.push({
      x: x - width / 2,
      y: y - height / 2,
      w: width,
    ...
    // 4) Find a random "blank" agent => (no Demand, no Supply, no Job)
  // Example snippet in BuildingSystem.js inside placeFridge(...) or wherever:
  const blankAgents = this.world.getEntitiesByComponents(['Position', 'Velocity', 'Follower'])
    .filter(id => 
      !this.world.hasComponent(id, 'Demand') &&
      !this.world.hasComponent(id, 'Supply') &&
      !this.world.hasComponent(id, 'Job')
    );

  if (blankAgents.length > 0) {
    const chosenAgent = blankAgents[Math.floor(Math.random() * blankAgents.length)];

    // Mark that agent with a "fridge" job
    this.world.addComponent(chosenAgent, 'Job', {
      jobType: 'fridge',
      // You can store extra data if needed:
      fridgeId: entityId // or something referencing THIS fridge's entity
    });

    console.log(`Agent ${chosenAgent} assigned to "fridge" job!`);
  } else {
    console.log('No blank agent available to do the fridge job.');
  }
  }
