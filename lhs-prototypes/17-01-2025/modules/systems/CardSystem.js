...
export class CardSystem {
  ...
  draw(p5, world) {
    ...
    for (let c of toRender) {
      ...
      // Draw the sprite icon instead of the emoji
      const spriteType = spriteMappings[c.type] ? c.type : 'test';
      const spriteSize = 20; 
      const spriteX = (c.width - spriteSize) / 2;
      const spriteY = (c.height - spriteSize) / 2 - 75;
      // Use sprite's draw function, passing the card as context (tile)
      spriteMappings[spriteType].draw(c, spriteX, spriteY, spriteSize);

      p5.fill(30);
      p5.textSize(20);
      p5.text(c.icon, c.width / 2, c.height / 2);
    
      // Price above card remains unchanged
      p5.fill(0);
      p5.textSize(12);
