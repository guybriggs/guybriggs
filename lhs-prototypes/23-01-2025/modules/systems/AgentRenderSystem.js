...
export class AgentRenderSystem {
  ...
  if (demand) {
    ...
    p5.pop();
      ...
      // (B) If this is a Farmer => pitchfork
      if (supply && supply.good === Goods.FARM_WORK) {
        p5.push();

        // 1) Move the "drawing origin" slightly to the right of the farmer
        const r = renderable.radius || 8;
        p5.translate(pos.x + r, pos.y - r * 0.5);

        // 2) Check if this farmer is standing on their own potato_seed tile
        let upDown = 0;  // default no vertical offset
        ...
        if (
          ...
          if (tile.type === 'potato_seed' && tile.claimed === entity) {
            // Wiggle up/down instead of rotation
            // Adjust 0.005 (speed) & 3 (amplitude) to taste
            upDown = Math.sin(p5.millis() * 0.005) * 3;
          }
        }

        // 3) Apply the upDown translation
        p5.translate(0, upDown);

        // 4) Draw the pitchfork (no rotation)
        p5.stroke('#A9A9A9');
        p5.strokeWeight(2);
...
if (inventory) {
  ...
  for (let ct of cropTypes) {
    ...
    if (count > 0) {
      ...
      p5.text(emoji, pos.x, pos.y - r - 15);
      p5.pop();
    }
  }
}
