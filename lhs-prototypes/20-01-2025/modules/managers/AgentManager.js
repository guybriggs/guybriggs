...
export class AgentManager {
  ...
  initializeAgents() {
    ...
    this.createFishDemanderAgents(10);
    ...
    // --- NEW: spawn two more "players," each with a fish-producer and fish-consumer follower.
  ...
  spawnTwoFishConsumers() {
    ...
    for (let i = 0; i < 2; i++) {
      // 2) Create a fish-consumer agent with a slight offset from the central point
      ...
      if (nameComp) {
        nameComp.firstName = `Fish Consumer ${i + 1}`;
      }

      // 4) (Optional) Add any additional components or customization here
      // For example, set specific emotions, add visual indicators, etc.
    }
  }
